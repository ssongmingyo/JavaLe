...
public class _10_TypeCasting {
	...
	public static void main(String[] args) {
		...
		// 1. 자동형변환1: byte 크기가 작은 타입에서 큰 타입으로는 자동혀연환이 일어난다. 
		short sNum1 = 10;
		
		// short은 2byte고 int는 4byte이기 때문에 sNum1이 자동으로  int 타입으로 변환된다 .
		int iNum1 = sNum1;
		
		// 2. 자동형변환2: 좀 더 세밀한 ㅠㅛ현범위를 갖는 타입으로 자동형변환이 일어난다. (정수 -> 실수)
		long lNum1 = 200L;
		...
		// 3. 강제형변환1: byte 크기가 큰 타입에서 작은 타입으로 변환할 때는 타입을 명시해줘야한다. 
		int iNum2 = 60;
		
		// int 타입인 iNum2가 short 타입보다 byte 크기가 크기 때문에 강제로 short 타입으로 변환한다. 
		short sNum2 = (short)iNum2;
		
		// 4. 강제형변환2: 덜 세밀한 표현범위를 갖는 타입으로 변환할 때도 타입을 명시해줘야한다.
		float fNum2 = 3.14f;
		...
		// 5. 연산에서의 형변환
		// 형변환 순서에 따라서 연산 중에 byte 크기가 더 큰 타입이나 좀 더 세밀한 표현범위를 갖는 타입으로 자동 형변환이 일어난다. 
		// 연산의 결과도 형변황된 타입으로 나타난다. 
